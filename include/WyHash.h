...
public:
    __device__ 
    static uint64_t wyhash64_v3_key2( const uint64_t key[2], uint64_t seed )
    {
        return mum( mum( key[0]^seed^_wyp0,  key[1]^seed^_wyp1  ), 16^_wyp4 );
    }

    __device__ 
    static uint64_t wyhash64_v3_key4( const uint64_t key[4], uint64_t seed )
    {
        return mum( mum( key[0]^seed^_wyp0,  key[1]^seed^_wyp1) ^ mum( key[2]^seed^_wyp2, key[3]^seed^_wyp4  ), 32^_wyp4 );
    }

    __device__ 
    static uint64_t wyhash64_v3_pair( uint64_t a, uint64_t b )
    {
     ...
     */
    __device__
    static uint64_t wyhash64_v1( uint64_t wyhash64 )
    {
