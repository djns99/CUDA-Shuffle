...
public:
    ...
    void shuffle( const ContainerType& in_container, ContainerType& out_container, uint64_t seed, uint64_t num ) override
    {
        ...
        if constexpr( device )
        {
            thrust::transform( counting, counting + num, tmp.begin(), [=] __device__( int64_t idx ) {
                GPURandomGenerator rng( seed, idx );
                return rng();
            } );
            thrust::sort_by_key( tmp.begin(), tmp.begin() + num, out_container.begin() );
        }
        else
        {
            thrust::transform( thrust::tbb::par, counting, counting + num, tmp.begin(), [=] __host__( int64_t idx ) {
                return WyHash::wyhash64_v3_pair( seed, idx );
            } );
            thrust::sort_by_key( thrust::tbb::par, tmp.begin(), tmp.begin() + num, out_container.begin() );
        }
    }
