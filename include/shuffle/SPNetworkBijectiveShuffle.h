...
__device__ static const uint8_t sp_sbox4[4] = { 0x3, 0x2, 0x0, 0x1 };
__device__ static const uint8_t sp_sbox16[16] = { 0x7, 0x5, 0xD, 0x8, 0x9, 0xA, 0xF, 0x3,
                                                  0x6, 0x1, 0xE, 0x0, 0xC, 0x4, 0x2, 0xB };
__device__ static const uint8_t sp_sbox256[256] = {
...
private:
    ...
    __host__ __device__ uint64_t doRound( const uint64_t state, const uint64_t round ) const
    {
        ...
        {
            output |= sp_sbox256[( state >> i ) & 0xFF] << i;
        }
        if( i <= num_bits - 4 )
        {
            output |= sp_sbox16[( state >> i ) & 0xF] << i;
            i += 4;
        }
        if( i <= num_bits - 2 )
        {
            output |= sp_sbox4[( state >> i ) & 0x3] << i;
        }
