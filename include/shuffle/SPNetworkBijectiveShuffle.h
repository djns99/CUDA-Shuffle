...
private:
    ...
    __host__ __device__ uint64_t reverseBits( uint64_t n ) const
    {
        ...
        return __brevll( n );
#else
        uint64_t byte_rev = __builtin_bswap64( n );
        uint64_t mask = 0x0101010101010101;
        uint64_t res = 0;
        for( uint64_t i = 8; i > 0; i -= 2 )
        {
            res |= ( byte_rev & mask ) << ( i - 1 );
            mask <<= 1;
        }
...
using SPNetworkBijectiveSortShuffle =
    BijectiveFunctionSortShuffle<SPNetworkBijectiveFunction<SP_DEFAULT_NUM_ROUNDS>, ContainerType, RandomGenerator>;

template <class ContainerType = thrust::device_vector<uint64_t>, class RandomGenerator = DefaultRandomGenerator>
using SPNetworkBijectiveScanShuffle =
    BijectiveFunctionScanShuffle<SPNetworkBijectiveFunction<SP_DEFAULT_NUM_ROUNDS>, ContainerType, RandomGenerator>;