...
public:
    ...
    {
        uint32_t state[2] = { uint32_t( val >> right_side_bits ), uint32_t( val & right_side_mask ) };
        for( int i = 0; i < num_rounds; i++ )
        {
            uint32_t hi;
...
template <class ContainerType = thrust::device_vector<uint64_t>, class RandomGenerator = DefaultRandomGenerator, uint64_t num_rounds = 24>
using PhiloxBijectiveScanShuffle =
    BijectiveFunctionScanShuffle<PhiloxBijectiveFunction<num_rounds>, ContainerType, RandomGenerator>;
...
using BasicPhiloxBijectiveScanShuffle =
    BasicBijectiveFunctionScanShuffle<PhiloxBijectiveFunction<>, ContainerType, RandomGenerator>;
template <class ContainerType = thrust::device_vector<uint64_t>, class RandomGenerator = DefaultRandomGenerator>
using TwoPassPhiloxBijectiveScanShuffle =
    MGPUBijectiveFunctionScanShuffle<PhiloxBijectiveFunction<>, ContainerType, RandomGenerator>;
