...
public:
    ...
    void init( uint64_t capacity, RandomGenerator& random_function )
    {
        ...
        {
            key[i] = { random_function(), {{random_function(), random_function()}, {random_function(), random_function()}, {random_function(), random_function()}, {random_function(), random_function()}}};
        }
    }
...
private:
    ...
    {
        uint64_t wyhash = wyhash64(input, key.wyhash);
        uint64_t lehmer = lehmer64(input, key.lehmer[round & 3][0], key.lehmer[round & 3][1]);
        return wyhash ^ lehmer;
    }

    /*
     * GGM Works by using a n -> 2n bit prbg and applying it once for every bit in the input
    ...
    __device__ RoundState lubyRackoff(RoundState state) const
    {
        ...
        const uint32_t delta = ggmPRF(state.left ^ gamma, key[3]);
        return {state.right ^ beta ^ delta, state.left ^ alpha ^ gamma};
    }
