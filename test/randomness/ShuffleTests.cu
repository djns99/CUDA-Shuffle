...
TYPED_TEST( RandomnessTests, SobolevaPermutationLengthStatistic )
{
    ...
    {
        this->runShuffle( shuffle_size );
        thrust::copy( this->shuffled_container.begin(),
                      this->shuffled_container.begin() + shuffle_size, h_results.begin() );
        ...
        for( uint64_t d = 2; d < num_dimensions; d++ )
        {
            ...
            std::cout << "Chi-Squared: " << chi_squared << " p-Score: " << p_value << " for D=" << d
                      << std::endl;
...
TYPED_TEST( RandomnessTests, TurningPointCount )
{
    ...
    for( uint64_t i = 0; i < num_samples; i++ )
    {
        ...
        for( uint64_t j = 1; j < shuffle_size - 1; j++ )
            if( ( h_results[j - 1] < h_results[j] ) == ( h_results[j + 1] < h_results[j] ) )
                num_tp++;

        const double z_score = ( (double)num_tp - expected ) / stddev;
        const double p_val = 1.0 - erf( abs( z_score ) / sqrt( 2.0 ) );
        std::cout << "z score: " << z_score << " and mean: " << expected << " stddev: " << stddev
                  << " tps: " << num_tp << " p-score: " << p_val << std::endl;
        ASSERT_GT( p_val, this->p_score_significance );
    }
}