...
BENCHMARK_TEMPLATE( benchmarkFunction, PrimeFieldBijectiveSortShuffle<thrust::device_vector<uint8_t>> )
    ->Apply( sortArgsGenerator );
...
// LubyRackoffBijectiveShuffle<thrust::device_vector<uint8_t>>)->Apply(argsGenerator); BENCHMARK_TEMPLATE(benchmarkFunction,
// LubyRackoffBijectiveSortShuffle<thrust::device_vector<uint8_t>>)->Apply(sortArgsGenerator); BENCHMARK_TEMPLATE(benchmarkFunction,
// GPUSwapShuffle<uint8_t>)->Apply(argsGenerator); Too much memory
