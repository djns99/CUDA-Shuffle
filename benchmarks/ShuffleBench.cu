...
BENCHMARK_TEMPLATE(benchmarkFunction, SPNetworkBijectiveSortShuffle<thrust::device_vector<uint8_t>>)->Apply(sortArgsGenerator);
BENCHMARK_TEMPLATE(benchmarkFunction, PrimeFieldBijectiveShuffle<thrust::device_vector<uint8_t>>)->Apply(argsGenerator);
BENCHMARK_TEMPLATE(benchmarkFunction, PrimeFieldBijectiveSortShuffle<thrust::device_vector<uint8_t>>)->Apply(sortArgsGenerator);
